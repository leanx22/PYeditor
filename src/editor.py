...
def main(args: list[str]):
    ...
    console.print_styled_message(
        ...
        "[bold cyan]2[/bold cyan]. Save changes\n"
        "[bold cyan]3[/bold cyan]. close file",
        "Actions",
        False,
        "cyan",
        True)

    file_action_input = console.show_prompt("[bold green]╰─❯[/bold green]",["1","2","3"], True)
    match file_action_input:
        case "1":
            console.print_message("1 selected")
            edit(file_content)
        case "2":
            console.print_message("2 selected")
        case "3":
            console.print_message("3 selected")
